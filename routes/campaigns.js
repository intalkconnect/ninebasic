...
// GET /api/v1/campaigns?tab=(all|active|finished|failed)&q=texto
  fastify.get('/', async (req) => {
    ...
    // filtro por “abas” do front
    if (tab === 'active') {
      where.push(`c.status IN ('queued','scheduled')`);
    } else if (tab === 'finished') {
      where.push(`c.status = 'finished'`);
    } else if (tab === 'failed') {
    ...
    // ⚠️ REGRA IMPORTANTE:
    // processed_count considera:
    //   - itens com message_id (mandados), OU
    //   - itens marcados como failed (falharam antes do wamid)
    // remaining = total_items - processed_count  (logo, falhas não entram em restantes)
    const sql = `
      SELECT
        c.id,
        c.name,
        ...
        -- processados = tem wamid OU falhou (terminal)
        COUNT(*) FILTER (WHERE ci.message_id IS NOT NULL OR ci.last_status = 'failed')::int
          AS processed_count
    ...
    // calcula remaining no Node para ficar explícito (e manter compatibilidade de payload)
