...
// GET /tickets/history/:id/pdf  -> gera e BAIXA o PDF do ticket (buffer, sem streaming)
fastify.get('/history/:id/pdf', async (req, reply) => {
  try {
    ...
    doc.on('end', () => {
      ...
      if (reply.sent) return; // segurança
      reply
    ...
    doc.end(); // dispara o 'end' -> envia o buffer na callback acima
  } catch (err) {
