...
const SENSITIVE_KEYS = new Set([
  ...
  'secret','apiKey','api_key','cpf','ssn','card','cvv'
]);
...
async function auditPlugin(fastify, opts) {
  ...
  const getActor = (req) => {
    ...
    const actor_name = req.user?.name || req.headers['x-user-name'] || null;
    return { actor_id, actor_name };
  };

  // expõe utilitário manual
  fastify.decorate('audit', async function auditLog(req, {
    action, resourceType, resourceId,
    beforeData, afterData, extra, requestBody, responseBody, statusCode
  }) {
    const db = getDb(req);
    ...
    `;
    const params = [
      actor_id, actor_name, method, path, statusCode ?? 200, ip, ua,
      action || null, resourceType || null, resourceId || null,
